...
string format_duration(long dur)
{
	...
	auto ms = duration_cast<milliseconds>(milsec);
	milsec -= duration_cast<microseconds>(ms);
	...
	ss << hour.count() << "h " << mins.count() << "m " << secs.count() << "s " << ms.count() << "ms " << milsec.count() << "\xE6s";
	return ss.str();
}
...
void GetEyeList(string filename, vector<TableList> &LeftTable, vector<TableList> &RightTable, vector<TableList> &LeftMinMax, vector<TableList> &RightMinMax, int imgsizeW)
{
...
#endif
					if (Table_tmp.xs < (3 + imgsizeW / 4) && Table_tmp.xe < (6 + imgsizeW / 2))
						LeftTable.push_back(Table_tmp);
					else
						RightTable.push_back(Table_tmp);
	...
	for (int i = 0; i < RightTable.size(); i++)
		vtmp.push_back(RightTable[i].xs);
	Table_tmp.xs = *std::min_element(vtmp.begin(), vtmp.end());
	value[1] = *std::max_element(vtmp.begin(), vtmp.end());
	...
	vtmp.clear();
	for (int i = 0; i < RightTable.size(); i++)
		vtmp.push_back(RightTable[i].ys);
	Table_tmp.ys = *std::min_element(vtmp.begin(), vtmp.end());
	value[2] = *std::max_element(vtmp.begin(), vtmp.end());
	...
	vtmp.clear();
	for (int i = 0; i < RightTable.size(); i++)
		vtmp.push_back(RightTable[i].xe);
	Table_tmp.xe = *std::min_element(vtmp.begin(), vtmp.end());
	value[3] = *std::max_element(vtmp.begin(), vtmp.end());
	...
	vtmp.clear();
	for (int i = 0; i < RightTable.size(); i++)
		vtmp.push_back(RightTable[i].ye);
	Table_tmp.ye = *std::min_element(vtmp.begin(), vtmp.end());
	value[4] = *std::max_element(vtmp.begin(), vtmp.end());
...
void BuildFeatureLoc(vector<TableList> &FeatureLoc, vector<TableList> LeftMinMax, vector<TableList> RightMinMax, int FaceWidth)
{
	...
	int ysmin = LeftMinMax[0].ys - offset;
	if (ysmin < 0)
		ysmin = 0;
	int xsmin = LeftMinMax[0].xs - offset;
	if (xsmin < 0)
		xsmin = 0;
	int yemin = LeftMinMax[0].ye - offset;
	if (yemin < 0)
		yemin = 0;
	int xemin = LeftMinMax[0].xe - offset;
	if (xemin < 0)
		xemin = 0;
	int ysmax = LeftMinMax[1].ys + offset;
	if (ysmax > 31)
		ysmax = 31;
	int xsmax = LeftMinMax[1].xs + offset;
	if (xsmax > 31)
		xsmax = 31;
	int yemax = LeftMinMax[1].ye + offset;
	if (yemax > 31)
		yemax = 31;
	int xemax = LeftMinMax[1].xe + offset;
	if (xemax > 31)
		xemax = 31;
	...
	ysmin = RightMinMax[0].ys - offset;
	if (ysmin < 0)
		ysmin = 0;
	xsmin = RightMinMax[0].xs - offset;
	if (xsmin < 0)
		xsmin = 0;
	yemin = RightMinMax[0].ye - offset;
	if (yemin < 0)
		yemin = 0;
	xemin = RightMinMax[0].xe - offset;
	if (xemin < 0)
		xemin = 0;
	ysmax = RightMinMax[1].ys + offset;
	if (ysmax > 31)
		ysmax = 31;
	xsmax = RightMinMax[1].xs + offset;
	if (xsmax > 31)
		xsmax = 31;
	yemax = RightMinMax[1].ye + offset;
	if (yemax > 31)
		yemax = 31;
	xemax = RightMinMax[1].xe + offset;
	if (xemax > 31)
		xemax = 31;
...
int Type3(TableList box, int **integral)
{
	...
	negbox.xs = box.xs;
	negbox.ye = box.ye;
	negbox.xe = box.xs + (box.xe - box.xs) / 2;
	posbox.ys = box.ys;
	posbox.xs = box.xs + (box.xe - box.xs) / 2;
	posbox.ye = box.ye;
	posbox.xe = box.xe;
...
int Type2(TableList box, int **integral)
{
	...
	negbox1.ys = box.ys;
	negbox1.xs = box.xs;
	negbox1.ye = box.ys + (box.ye - box.ys) / 2;
	negbox1.xe = box.xs + (box.xe - box.xs) / 2;
	negbox2.ys = box.ys + (box.ye - box.ys) / 2;
	negbox2.xs = box.xs + (box.xe - box.xs) / 2;
	negbox2.ye = box.ye;
	negbox2.xe = box.xe;
	posbox1.ys = box.ys;
	posbox1.xs = box.xs + (box.xe - box.xs) / 2;
	posbox1.ye = box.ys + (box.ye - box.ys) / 2;
	posbox1.xe = box.xe;
	posbox2.ys = box.ys + (box.ye - box.ys) / 2;
	posbox2.xs = box.xs;
	posbox2.ye = box.ye;
	posbox2.xe = box.xs + (box.xe - box.xs) / 2;
...
int Type1(TableList box, int **integral)
{
	...
	posbox1.ys = box.ys;
	posbox1.xs = box.xs;
	posbox1.ye = box.ys + (box.ye - box.ys) / 3;
	posbox1.xe = box.xe;
	posbox2.ys = box.ys + (box.ye - box.ys) * 2 / 3;
	posbox2.xs = box.xs;
	posbox2.ye = box.ye;
	posbox2.xe = box.xe;
	negbox1.ys = box.ys + (box.ye - box.ys) / 3;
	negbox1.xs = box.xs;
	negbox1.ye = box.ys + (box.ye - box.ys) * 2 / 3;
	negbox1.xe = box.xe;
	int negRigion1 = integral[negbox1.ys][negbox1.xs] + integral[negbox1.ye][negbox1.xe] - integral[negbox1.ys][negbox1.xe] - integral[negbox1.ye][negbox1.xs];
	int posRigion1 = integral[posbox1.ys][posbox1.xs] + integral[posbox1.ye][posbox1.xe] - integral[posbox1.ys][posbox1.xe] - integral[posbox1.ye][posbox1.xs];
	int posRigion2 = integral[posbox2.ys][posbox2.xs] + integral[posbox2.ye][posbox2.xe] - integral[posbox2.ys][posbox2.xe] - integral[posbox2.ye][posbox2.xs];
...
int Type0(TableList box, int **integral)
{
	...
	negbox1.ys = box.ys + (box.ye - box.ys) / 4;
	negbox1.xs = box.xs + (box.xe - box.xs) / 4;
	negbox1.ye = box.ys + (box.ye - box.ys) * 3 / 4;
	negbox1.xe = box.xs + (box.xe - box.xs) * 3 / 4;
	posbox1.ys = box.ys;
	posbox1.xs = box.xs;
	posbox1.ye = box.ye;
	posbox1.xe = box.xs + (box.xe - box.xs) / 4;
	posbox2.ys = box.ys;
	posbox2.xs = box.xs + (box.xe - box.xs) * 3 / 4;
	posbox2.ye = box.ye;
	posbox2.xe = box.xe;
	posbox3.ys = box.ys;
	posbox3.xs = box.xs + (box.xe - box.xs) / 4;
	posbox3.ye = box.ys + (box.ye - box.ys) / 4;
	posbox3.xe = box.xs + (box.xe - box.xs) * 3 / 4;
	posbox4.ys = box.ys + (box.ye - box.ys) * 3 / 4;
	posbox4.xs = box.xs + (box.xe - box.xs) / 4;
	posbox4.ye = box.ye;
	posbox4.xe = box.xs + (box.xe - box.xs) * 3 / 4;
	int negRigion1 = integral[negbox1.ys][negbox1.xs] + integral[negbox1.ye][negbox1.xe] - integral[negbox1.ys][negbox1.xe] - integral[negbox1.ye][negbox1.xs];
	int posRigion1 = integral[posbox1.ys][posbox1.xs] + integral[posbox1.ye][posbox1.xe] - integral[posbox1.ys][posbox1.xe] - integral[posbox1.ye][posbox1.xs];
	int posRigion2 = integral[posbox2.ys][posbox2.xs] + integral[posbox2.ye][posbox2.xe] - integral[posbox2.ys][posbox2.xe] - integral[posbox2.ye][posbox2.xs];
					 ...
					 vector<TableList> LeftTable, vector<TableList> RightTable)
{
	...
	for (int i = 0; i < FeatureLoc.size() * 4; i++)
	{
		...
		box_tmp.id = k;
		if (i % 4 == 0)
			vtmp.fv = Type0(FeatureLoc[i / 4], integral);
		else if (i % 4 == 1)
			vtmp.fv = Type1(FeatureLoc[i / 4], integral);
		else if (i % 4 == 2)
			vtmp.fv = Type2(FeatureLoc[i / 4], integral);
		else
			vtmp.fv = Type3(FeatureLoc[i / 4], integral);

		vtmp.hit = 0;
		j = 0;
		while (j < img_cnt && j < LeftTable.size() && LeftTable[j].id != k)
			j++;
		if (j < LeftTable.size() && LeftTable[j].id == k &&
			box_tmp.xs >= LeftTable[j].xs &&
			box_tmp.ys >= LeftTable[j].ys &&
			box_tmp.xe <= LeftTable[j].xe &&
			box_tmp.ye <= LeftTable[j].ye)
			vtmp.hit = 1;

		j = 0;
		while (j < img_cnt && j < RightTable.size() && RightTable[j].id != k)
			j++;
						   ...
						   vector<FeatureThreshold> &ThresholdTable, int img_cnt)
{
	...
	{
		if (fid % 100000 == 0 || fid == FeatureLen - 1)
			cout << fid << " ";
		//		cout << fid << " ";
		int totalhit = 0;
		int totalsample = 0;
		...
		for (int img = 0; img < img_cnt; img++)
		{
			...
			if (FeatureImageAll[fid][img].fv > 0)
			{
				...
				if (totalhit == totalsample)
					EN = 1;
				else
					EN = ((double)errorNeg) / ((double)(totalsample - totalhit));
				if (totalhit == 0)
					EP = 1;
				else
					EP = ((double)errorPos) / ((double)totalhit);
				gini = EN * EN + EP * EP;
				if (gini < minGini)
				{
					save = th;
...
void initWeights(vector<int> FeatureHit, double **weights, int FeatureLen, int img_cnt)
{
	...
	for (int fid = 0; fid < FeatureLen; fid++)
	{
		...
		for (int img = 0; img < img_cnt; img++)
		{
			...
			{
				if (poscnt == 0)
					weights[fid][img] = 1.0 / double(negcnt);
				else
					weights[fid][img] = 1.0 / double(negcnt * 2);
			}
			else
			{
				if (negcnt == 0)
					weights[fid][img] = 1.0 / double(poscnt);
				else
					weights[fid][img] = 1.0 / double(poscnt * 2);
			}
		}
	}
}
...
void updateWeights(double **weights, vector<int> ThresholdHit, vector<int> FeatureHit, int FeatureLen, int img_cnt, int minIndex, double beta)
{
	...
	for (int img = 0; img < img_cnt; img++)
	{
		if (FeatureHit[minIndex * img_cnt + img] == ThresholdHit[minIndex * img_cnt + img])
...
void FilterFeatures(string WorkDrive, FeatureValue **FeatureImageAll, vector<TableList> &FeatureLoc, int img_cnt, int percentage)
{
	...
	for (int i = 0; i < NewFeatureImageAllIndex.size() * 4; i++)
		NewFeatureImageAll[i] = new FeatureValue[img_cnt];
	int newIdx = 0;
...
int main(int argc, char **argv)
{
	...
	{
		WorkDrive = argv[1];
		img_start = atoi(argv[2]);
		img_end = atoi(argv[3]);
		img_cnt = img_end - img_start + 1;
	}
#ifdef DEBUG
	...
	for (int i = 0; i < imgsizeH; i++)
		img[i] = new int[sizeof(int) * imgsizeW];
	int **integral = new int *[imgsizeH];
	for (int i = 0; i < imgsizeH; i++)
		integral[i] = new int[sizeof(int) * imgsizeW];
	...
	for (int i = 0; i < FeatureLoc.size() * 4; i++)
		FeatureImageAll[i] = new FeatureValue[img_cnt];
	...
	for (int i = 0; i < FeatureLen; i++)
		delete[] FeatureImageAll[i];
	delete[] FeatureImageAll;

	cout << "Release FeatureImage......." << endl;

	double **weights = new double *[FeatureLen];
	for (int i = 0; i < FeatureLen; i++)
		weights[i] = new double[img_cnt];
